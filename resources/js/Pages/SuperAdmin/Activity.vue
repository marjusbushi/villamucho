...
const ACTIONS = {
    'tenant.create': { label: 'Hotel u krijua', icon: Plus, tone: 'blue' },
    'tenant.switch': { label: 'Hyrje në hotel', icon: LogIn, tone: 'neutral' },
    'tenant.subscription.update': { label: 'Abonimi u përditësua', icon: CreditCard, tone: 'emerald' },
    'tenant.integration.update': { label: 'Integrim u ndryshua', icon: Plug, tone: 'violet' },
    'tenant.domain.create': { label: 'Domain u shtua', icon: Globe2, tone: 'blue' },
    'tenant.domain.delete': { label: 'Domain u hoq', icon: Globe2, tone: 'red' },
    'tenant.domain.primary': { label: 'Domain primar u ndryshua', icon: Globe2, tone: 'blue' },
    'tenant.status': { label: 'Statusi i hotelit', icon: Power, tone: 'amber' },
};
...
function dayLabel(value) {
    ...
    if (diff === 0) return `Sot · ${new Intl.DateTimeFormat('sq-AL', { day: 'numeric', month: 'long' }).format(valueDate)}`;
    if (diff === 1) return 'Dje';
    return new Intl.DateTimeFormat('sq-AL', { day: 'numeric', month: 'long', year: 'numeric' }).format(valueDate);
}
...
function exportRows() {
    const header = ['Data', 'Veprimi', 'Hoteli', 'Përmbledhja', 'Administratori', 'Email', 'IP'];
...
<template>
    ...
    <SuperAdminLayout title="Aktiviteti — Lora Control Panel">
        <div class="mx-auto max-w-[1480px] space-y-5">
            <div>
                <div class="text-xs text-neutral-400"><span>Control Panel</span><span class="mx-2">/</span><span>Aktiviteti</span></div>
                <div class="mt-3 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h1 class="text-3xl font-semibold tracking-tight text-neutral-950">Aktiviteti i platformës</h1>
                        <p class="mt-2 text-sm text-neutral-500">Audit i qartë i çdo veprimi administrativ.</p>
                    </div>
            ...
            <section class="grid gap-4 sm:grid-cols-3">
                <article class="flex items-center gap-3 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm shadow-neutral-200/30">
                    ...
                    <div><p class="text-2xl font-semibold text-neutral-900">{{ stats.actions_24h }}</p><p class="text-xs text-neutral-500">Veprime në 24 orët e fundit</p></div>
                </article>
                <article class="flex items-center gap-3 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm shadow-neutral-200/30">
                    <span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-blue-50 text-blue-700"><Building2 class="h-5 w-5" /></span>
                    <div><p class="text-2xl font-semibold text-neutral-900">{{ stats.hotels_24h }}</p><p class="text-xs text-neutral-500">Hotele të prekura</p></div>
                </article>
                <article class="flex items-center gap-3 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm shadow-neutral-200/30">
                    ...
                    <div><p class="text-2xl font-semibold text-neutral-900">{{ stats.admins_24h }}</p><p class="text-xs text-neutral-500">Administratorë aktivë</p></div>
                </article>
            </section>
            ...
            <section class="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm shadow-neutral-200/30">
                <div class="flex items-center justify-between gap-3 border-b border-neutral-200 px-5 py-4">
                    <div><h2 class="text-lg font-semibold text-neutral-900">Gjurma e veprimeve</h2><p class="mt-1 text-sm text-neutral-500">Veprimet janë të pandryshueshme dhe pa sekrete.</p></div>
                    <span class="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-semibold text-neutral-600">{{ logs.total }} rezultate</span>
                </div>
                ...
                <div class="grid gap-3 border-b border-neutral-200 px-5 py-4 lg:grid-cols-[minmax(260px,1fr)_220px_200px_170px]">
                    <label class="relative block">
                        ...
                        <input v-model="search" type="search" class="w-full rounded-xl border-neutral-300 py-2.5 pl-10 pr-3 text-sm" placeholder="Kërko hotel, administrator ose IP…" />
                    </label>
                    ...
                    <select v-model="hotelFilter" class="rounded-xl border-neutral-300 py-2.5 text-sm text-neutral-700" @change="applyFilters">
                        <option value="">Të gjitha hotelet</option>
                ...
                <div v-else class="flex flex-col items-center gap-2 px-5 py-16 text-center">
                    ...
                    <p class="text-sm font-medium text-neutral-700">Nuk u gjet asnjë veprim</p>
                    <p class="text-xs text-neutral-500">Ndrysho kërkimin ose filtrat e zgjedhur.</p>
                </div>

                <div v-if="logs.last_page > 1" class="flex items-center justify-between border-t border-neutral-200 px-5 py-3 text-sm">
                    <button class="rounded-lg border border-neutral-200 px-3 py-1.5 text-neutral-600 disabled:opacity-40" :disabled="!logs.prev_page_url" @click="go(logs.prev_page_url)">← Të mëparshme</button>
                    <span class="text-xs text-neutral-500">Faqja {{ logs.current_page }} nga {{ logs.last_page }} · {{ logs.total }} veprime</span>
                    <button class="rounded-lg border border-neutral-200 px-3 py-1.5 text-neutral-600 disabled:opacity-40" :disabled="!logs.next_page_url" @click="go(logs.next_page_url)">Në vazhdim →</button>
                </div>
            </section>
        </div>
            ...
            <div v-if="selectedLog" class="fixed inset-0 z-50 bg-neutral-950/40" @click.self="selectedLog = null">
                <aside role="dialog" aria-modal="true" class="ml-auto flex h-full w-full max-w-lg flex-col bg-white shadow-2xl">
                    <div class="flex items-start justify-between border-b border-neutral-200 px-6 py-5">
                        <div><h2 class="text-lg font-semibold text-neutral-900">Detaji i veprimit</h2><p class="mt-1 text-sm text-neutral-500">Audit log · #{{ selectedLog.id }}</p></div>
                        <button type="button" class="rounded-xl p-2 text-neutral-400 hover:bg-neutral-100 hover:text-neutral-700" aria-label="Mbyll detajin" @click="selectedLog = null"><X class="h-5 w-5" /></button>
                    </div>
                    ...
                    <div class="flex-1 space-y-6 overflow-y-auto p-6">
                        ...
                        <section><h3 class="text-xs font-semibold uppercase tracking-[0.1em] text-neutral-400">Kush dhe kur</h3><div class="mt-3 divide-y divide-neutral-100 rounded-xl border border-neutral-200"><div class="flex justify-between gap-4 px-4 py-3 text-sm"><span class="text-neutral-500">Administratori</span><strong class="text-right text-neutral-900">{{ selectedLog.actor }}</strong></div><div class="flex justify-between gap-4 px-4 py-3 text-sm"><span class="text-neutral-500">Email</span><strong class="break-all text-right text-neutral-900">{{ selectedLog.actor_email || '—' }}</strong></div><div class="flex justify-between gap-4 px-4 py-3 text-sm"><span class="text-neutral-500">Koha</span><strong class="text-right text-neutral-900">{{ fullDate(selectedLog.created_at) }}</strong></div><div class="flex justify-between gap-4 px-4 py-3 text-sm"><span class="text-neutral-500">IP</span><strong class="text-right text-neutral-900">{{ selectedLog.ip_address || '—' }}</strong></div></div></section>

                        <section><h3 class="text-xs font-semibold uppercase tracking-[0.1em] text-neutral-400">Veprimi</h3><div class="mt-3 divide-y divide-neutral-100 rounded-xl border border-neutral-200"><div class="flex justify-between gap-4 px-4 py-3 text-sm"><span class="text-neutral-500">Objekti</span><strong class="text-right text-neutral-900">{{ selectedLog.tenant || 'Platforma' }}</strong></div><div class="flex justify-between gap-4 px-4 py-3 text-sm"><span class="text-neutral-500">Rezultati</span><strong class="text-right text-emerald-700">U përfundua</strong></div><div class="flex justify-between gap-4 px-4 py-3 text-sm"><span class="text-neutral-500">Përmbledhje</span><strong class="max-w-[280px] text-right text-neutral-900">{{ selectedLog.summary || actionMeta(selectedLog.action).label }}</strong></div></div></section>

                        <div class="flex items-start gap-3 rounded-xl bg-blue-50 px-4 py-3 text-blue-700"><ShieldCheck class="mt-0.5 h-5 w-5 shrink-0" /><div><p class="text-sm font-semibold">Pa të dhëna sekrete</p><p class="mt-1 text-xs leading-5 text-blue-600">Audit-i ruan vetëm fushat e ndryshuara, jo kredencialet.</p></div></div>
                    </div>

                    <div class="flex justify-end border-t border-neutral-200 bg-neutral-50/70 px-6 py-4">
                        <button type="button" class="inline-flex items-center gap-2 rounded-xl border border-neutral-200 bg-white px-4 py-2 text-sm font-semibold text-neutral-700 hover:bg-neutral-50" @click="selectedLog = null">Mbyll</button>
                    </div>
                </aside>
            </div>
        </Teleport>
