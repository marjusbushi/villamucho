    ...
    <SuperAdminLayout title="Provider Events — Lora Control Panel">
        <div class="mx-auto max-w-7xl space-y-6">
            ...
            <section class="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
                ...
                <div class="overflow-x-auto"><table class="min-w-full divide-y divide-neutral-200 text-sm"><thead class="bg-neutral-50 text-left text-xs uppercase tracking-wide text-neutral-500"><tr><th class="px-5 py-3 font-semibold">Event ID</th><th class="px-5 py-3 font-semibold">Provider / Tipi</th><th class="px-5 py-3 font-semibold">Hoteli</th><th class="px-5 py-3 font-semibold">Lidhjet</th><th class="px-5 py-3 font-semibold">Koha</th><th class="px-5 py-3 font-semibold">Statusi</th><th class="px-5 py-3 text-right font-semibold">Veprimi</th></tr></thead><tbody class="divide-y divide-neutral-100"><tr v-for="event in events.data" :key="event.id" class="hover:bg-neutral-50"><td class="px-5 py-4"><Link :href="`/super-admin/billing/provider-events/${event.id}`" class="font-mono text-xs font-semibold text-emerald-700 no-underline">{{ event.external_id }}</Link></td><td class="px-5 py-4"><strong class="block text-neutral-900">{{ event.provider }}</strong><span class="mt-1 block font-mono text-xs text-neutral-500">{{ event.event_type }}</span></td><td class="px-5 py-4"><Link v-if="event.tenant" :href="`/super-admin/tenants/${event.tenant.id}`" class="text-neutral-700 no-underline hover:text-emerald-700">{{ event.tenant.name }}</Link><span v-else>Platformë</span></td><td class="px-5 py-4 text-xs"><Link v-if="event.invoice" :href="`/super-admin/billing/invoices/${event.invoice.id}`" class="block font-semibold text-emerald-700 no-underline">{{ event.invoice.number }}</Link><Link v-if="event.payment" :href="`/super-admin/billing/payments/${event.payment.id}`" class="mt-1 block text-neutral-500 no-underline">{{ event.payment.number }}</Link><Link v-if="event.attempt" :href="`/super-admin/billing/payment-attempts/${event.attempt.id}`" class="mt-1 block text-neutral-500 no-underline">Tentativa #{{ event.attempt.id }}</Link><span v-if="!event.invoice && !event.payment && !event.attempt">—</span></td><td class="whitespace-nowrap px-5 py-4 text-neutral-500">{{ dateTime(event.occurred_at) }}</td><td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-medium" :class="statusClass(event.status)">{{ event.status }}</span><span v-if="event.attempt_count" class="ml-2 text-xs text-neutral-400">retry {{ event.attempt_count }}</span></td><td class="px-5 py-4 text-right"><button v-if="event.status === 'failed'" class="inline-flex items-center gap-1 rounded-lg border border-neutral-300 px-3 py-1.5 text-xs font-semibold text-neutral-700 hover:bg-neutral-50" @click="retry(event)"><RotateCw class="h-3.5 w-3.5" /> Retry</button><Link v-else :href="`/super-admin/billing/provider-events/${event.id}`" class="text-xs font-semibold text-emerald-700 no-underline">Hap</Link></td></tr><tr v-if="!events.data.length"><td colspan="7" class="px-5 py-12 text-center"><p class="font-medium text-neutral-700">Nuk ka ende provider events.</p><p class="mt-1 text-sm text-neutral-400">Eventet do të shfaqen kur të lidhet provider-i online.</p></td></tr></tbody></table></div>
