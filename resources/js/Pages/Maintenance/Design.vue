        ...
        <Teleport to="body">
            <div v-if="reportOpen" class="fixed inset-0 z-50 flex items-end justify-center bg-neutral-950/50 sm:items-center sm:p-4" @click.self="reportOpen = false">
                <form class="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl" @submit.prevent="submitReport">
                    <div class="sticky top-0 z-10 flex items-center justify-between border-b border-neutral-200 bg-white px-5 py-4"><div class="flex items-center gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-950 text-white"><Wrench class="h-4 w-4" /></span><div><h2 class="text-lg font-bold text-neutral-950">{{ t('maintenance.reportIssue') }}</h2><p class="mt-0.5 text-xs text-neutral-500">{{ t('maintenance.reportHint') }}</p></div></div><button type="button" class="rounded-lg p-2 text-neutral-400 hover:bg-neutral-100" @click="reportOpen = false"><X class="h-4 w-4" /></button></div>
                    <div class="grid gap-3.5 p-5 sm:grid-cols-2">
