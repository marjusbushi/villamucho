    ...
    <AppLayout>
        ...
        <Teleport to="body">
            <div v-if="drawerOpen && selected" class="fixed inset-0 z-50 bg-neutral-950/40 backdrop-blur-[1px]" @click.self="drawerOpen = false">
                <aside class="absolute inset-y-0 right-0 flex w-full max-w-[600px] flex-col overflow-hidden bg-neutral-50 shadow-2xl">
                    <div class="flex shrink-0 items-center justify-between border-b border-neutral-200 bg-white px-5 py-4"><div class="flex items-center gap-2"><span class="rounded-md bg-neutral-100 px-2 py-1 text-[11px] font-bold text-neutral-500">#MNT-{{ selected.id }}</span><span class="inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold ring-1 ring-inset" :class="priorityClass(selected.priority)">{{ t(`maintenance.${selected.priority}`) }}</span><span class="inline-flex items-center gap-1.5 rounded-full bg-neutral-100 px-2.5 py-1 text-[11px] font-semibold text-neutral-700"><span class="h-2 w-2 rounded-full" :class="statusClass(selected.status)"></span>{{ statusLabel(selected.status) }}</span></div><button class="rounded-xl p-2 text-neutral-400 transition hover:bg-neutral-100 hover:text-neutral-700" @click="drawerOpen = false"><X class="h-5 w-5" /></button></div>
                    <div class="flex-1 overflow-y-auto p-5 sm:p-6"><div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><div class="flex items-start justify-between gap-4"><div><p class="text-xs font-semibold uppercase tracking-wider text-emerald-700">{{ location(selected) }} · {{ categoryLabel(selected.category) }}</p><h2 class="mt-2 text-2xl font-bold leading-tight text-neutral-950">{{ selected.title }}</h2><p class="mt-2 text-xs text-neutral-400">{{ t('maintenance.reportedBy') }} {{ selected.reporter?.name || '—' }} · {{ formatDate(selected.created_at) }}</p></div><span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-950 text-white"><Wrench class="h-5 w-5" /></span></div></div>
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-3"><div class="rounded-xl border border-neutral-200 bg-white p-3.5 shadow-sm"><span class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">{{ t('maintenance.assignee') }}</span><p class="mt-2 flex items-center gap-2 text-sm font-semibold"><span class="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-50 text-xs text-emerald-800">{{ (selected.assignee?.name || '?').charAt(0) }}</span>{{ selected.assignee?.name || t('maintenance.unassigned') }}</p></div><div class="rounded-xl border border-neutral-200 bg-white p-3.5 shadow-sm"><span class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">SLA</span><p class="mt-2 flex items-center gap-2 text-sm font-semibold"><span class="flex h-7 w-7 items-center justify-center rounded-full bg-blue-50 text-blue-700"><Clock3 class="h-3.5 w-3.5" /></span>{{ sla(selected) }}</p></div></div>
                        <div class="mt-4 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><h3 class="text-xs font-bold uppercase tracking-wider text-neutral-400">{{ t('maintenance.report') }}</h3><p class="mt-3 whitespace-pre-line text-sm leading-6 text-neutral-700">{{ selected.description || '—' }}</p></div>
                        <div v-if="selected.asset_name || selected.asset_code" class="mt-4 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><span class="text-[10px] font-bold uppercase tracking-wider text-neutral-400">{{ t('maintenance.asset') }}</span><div class="mt-2 flex items-center justify-between gap-3"><p class="text-sm font-semibold text-neutral-900">{{ selected.asset_name || '—' }}</p><span v-if="selected.asset_code" class="rounded-md bg-neutral-100 px-2 py-1 font-mono text-[11px] text-neutral-500">{{ selected.asset_code }}</span></div></div>

                        <div v-if="permissions.update" class="mt-4 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><label class="text-xs font-bold uppercase tracking-wider text-neutral-400">{{ t('maintenance.assignee') }}</label><div class="mt-3 flex gap-2"><select v-model="assignedTo" class="min-w-0 flex-1 rounded-xl border-neutral-200 bg-neutral-50 text-sm"><option value="">{{ t('maintenance.chooseTechnician') }}</option><option v-for="person in staff" :key="person.id" :value="person.id">{{ person.name }}</option></select><button class="rounded-xl bg-neutral-900 px-4 text-sm font-semibold text-white disabled:opacity-40" :disabled="!assignedTo" @click="assignIssue">{{ t('maintenance.assign') }}</button></div></div>

                        <div class="mt-4 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><h3 class="text-xs font-bold uppercase tracking-wider text-neutral-400">{{ t('maintenance.attachments') }}</h3><div class="mt-3 space-y-2"><a v-for="file in selected.attachments" :key="file.id" :href="file.url" class="flex items-center gap-2 rounded-xl border border-neutral-200 p-3 text-sm font-medium text-emerald-800 transition hover:bg-emerald-50"><Image class="h-4 w-4" />{{ file.name }}</a><p v-if="!selected.attachments.length" class="rounded-xl bg-neutral-50 px-3 py-4 text-center text-sm text-neutral-400">{{ t('maintenance.noAttachments') }}</p></div><div v-if="permissions.update" class="mt-3 flex gap-2"><input type="file" class="min-w-0 flex-1 text-xs" @change="uploadFile = $event.target.files[0]" /><button class="rounded-lg border border-neutral-300 px-3 py-2 text-xs font-semibold disabled:opacity-40" :disabled="!uploadFile" @click="uploadAttachment"><FileUp class="inline h-4 w-4" /> {{ t('maintenance.upload') }}</button></div></div>

                        <div class="mt-4 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><h3 class="text-xs font-bold uppercase tracking-wider text-neutral-400">{{ t('maintenance.timeline') }}</h3><div class="relative mt-4 space-y-4 before:absolute before:bottom-2 before:left-[5px] before:top-2 before:w-px before:bg-neutral-200"><div v-for="event in selected.events" :key="event.id" class="relative pl-6"><span class="absolute left-0 top-1.5 h-[11px] w-[11px] rounded-full border-2 border-white bg-emerald-500 ring-1 ring-emerald-200"></span><p class="text-sm font-semibold text-neutral-800">{{ event.to_status ? statusLabel(event.to_status) : event.type }}</p><p class="mt-0.5 text-xs leading-5 text-neutral-500">{{ event.user?.name || '—' }} · {{ formatDate(event.created_at) }}<span v-if="event.note" class="mt-1 block text-neutral-700">{{ event.note }}</span></p></div></div></div>
                    </div>
                    <div v-if="permissions.update" class="shrink-0 border-t border-neutral-200 bg-white p-4 shadow-[0_-10px_30px_rgba(0,0,0,0.04)]"><textarea v-if="nextStatus(selected.status)" v-model="statusNote" rows="2" class="mb-3 w-full resize-none rounded-xl border-neutral-200 bg-neutral-50 text-sm focus:bg-white" :placeholder="selected.status === 'in_progress' ? t('maintenance.resolutionRequired') : t('maintenance.actionNote')"></textarea><div class="grid gap-2 sm:grid-cols-2"><button v-if="selected.room" class="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-neutral-300 text-sm font-semibold text-neutral-700 hover:bg-neutral-50" @click="toggleRoomBlock">{{ selected.room_blocked ? t('maintenance.releaseRoom') : t('maintenance.blockRoom') }}</button><button v-if="nextStatus(selected.status)" :disabled="selected.status === 'in_progress' && !statusNote.trim()" class="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-900 text-sm font-semibold text-white shadow-sm hover:bg-emerald-950 disabled:opacity-40" @click="advanceStatus"><Hammer class="h-4 w-4" />{{ t(`maintenance.${nextAction(selected.status)}`) }}</button></div></div>
                </aside>
            </div>
        </Teleport>
