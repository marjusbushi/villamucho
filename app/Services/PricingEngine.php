...
class PricingEngine
{
    ...
    public static function forRange(RoomType $type, Carbon $from, Carbon $to): array
    {
        ...
        // Demand: active reservations on serviceable rooms, property-wide
        // (pending holds inventory on purpose — an unpaid POK hold blocks the
        // room until pok:release-unpaid frees it).
        $reservations = Reservation::whereNotIn('status', ['cancelled', 'checked_out'])
            ->whereIn('room_id', $propertyRoomIds)
        ...
        for ($d = $from->copy(); $d->lte($to); $d->addDay()) {
            ...
            $bookedType = $reservations->filter($nightCovers)
                ->whereIn('room_id', $typeRoomIds)->pluck('room_id')->unique()->count();
            $bookedProperty = $reservations->filter($nightCovers)
    ...
    private static function compute(RoomType $type, Carbon $date, array $ctx): array
    {
        ...
        if (! $isPast && $reference > 0 && $typeTotal > 0) {
            ...
            if (abs($occPct) >= 0.05) {
                $factors[] = ['key' => 'occupancy', 'label' => sprintf('Zënia %d/%d dhoma (%s%%)', $ctx['type_booked'], $typeTotal, $occ), 'pct' => round($occPct, 1)];
            }

            // 2. Pickup pace — how fast this night filled since the baseline
            // snapshot (property-level, normalized to a 7-day window).
            if ($ctx['baseline_booked'] !== null && $ctx['baseline_span'] >= 3) {
                $pickup7 = ($ctx['property_booked'] - $ctx['baseline_booked']) * 7 / $ctx['baseline_span'];
                $pacePct = self::paceCurve($pickup7) * $mult;
                if (abs($pacePct) >= 0.05) {
                    $factors[] = ['key' => 'pace', 'label' => sprintf('Ritmi: %+.0f rezervime/javë', $pickup7), 'pct' => round($pacePct, 1)];
                }
            }
            ...
            if (abs($leadPct) >= 0.05) {
                $factors[] = ['key' => 'lead_time', 'label' => $leadPct < 0
            ...
            if (in_array((int) $date->dayOfWeekIso, [5, 6], true)) {
                $factors[] = ['key' => 'dow', 'label' => 'Fundjavë (Pre/Sht)', 'pct' => round(8.0 * $mult, 1)];
            }

            // 5. Events — the owner's explicit uplifts, NOT scaled by strategy.
            foreach ($ctx['events'] as $event) {
                if ($event->uplift_pct !== null && (float) $event->uplift_pct != 0.0) {
                    $factors[] = ['key' => 'event', 'label' => $event->name, 'pct' => round((float) $event->uplift_pct, 1)];
                }
            }
        }

        // Compose multiplicatively, then apply the anti-nag + guardrails.
        $suggested = $reference;
        foreach ($factors as $f) {
            $suggested *= 1 + $f['pct'] / 100;
        }
        $suggested = round($suggested, 2);

        // Far-future empty days: never nag a discount (suggestion collapses to current).
        if ($suggested < $reference && $daysUntil > self::DISCOUNT_HORIZON_DAYS) {
            $suggested = round($current, 2);
        }

        // Owner guardrails: min/max per room type (breakdown marks the clamp).
        $clamped = null;
        if ($type->max_price !== null && $suggested > (float) $type->max_price) {
            $suggested = round((float) $type->max_price, 2);
            $clamped = 'max';
        }
        if ($type->min_price !== null && $suggested < (float) $type->min_price) {
            $suggested = round((float) $type->min_price, 2);
            $clamped = 'min';
        }
